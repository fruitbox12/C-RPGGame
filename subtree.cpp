...
/*
	findSubTree() will return a pointer to the SubTree containing the key,
 ...
 */
 ...
 /*
	findSubtreeRecursive() will return a pointer to the Tree containing the root Node key,
	or nullptr if the key is not found anywhere in the Tree
 */
template <typename T1, typename T2>
...
/*
	findNodeRecursive() will return a pointer to the Node (nextNode parm) containing the key,
	or nullptr if the key is not found anywhere in the Tree
*/
template <typename T1, typename T2>
...
template <typename T1, typename T2>
void SubTree<T1, T2>::preOrderTraversalRecursive(SubTree<T1, T2>* nextTree, std::vector<Tree<T1, T2>*> &traverse) {
	if (nextTree != nullptr) {
		// Omit the root Tree if travered already
		if (nextTree->getLevel() > 0 && (traverse.size() == 0 || (traverse.size() > 0 && nextTree != traverse[traverse.size() - 1]))) {
			traverse.push_back(static_cast<Tree<T1,T2>*>(nextTree));
		}

		// Recursively traverse the left (previous) tree
		preOrderTraversalRecursive(nextTree->getPrevSubtree(), traverse);

		// Recursively traverse the right (next) tree
		preOrderTraversalRecursive(nextTree->getNextSubtree(), traverse);
	}
}

template <typename T1, typename T2>
void SubTree<T1, T2>::inOrderTraversalRecursive(SubTree<T1, T2>* nextTree, std::vector<Tree<T1, T2>*> &traverse) {
	if (nextTree != nullptr) {
		// Recursively traverse the left (previous) tree
		inOrderTraversalRecursive(nextTree->getPrevSubtree(), traverse);
		...
		// Recursively traverse the right (next) tree
		inOrderTraversalRecursive(nextTree->getNextSubtree(), traverse);
	}
}

template <typename T1, typename T2>
void SubTree<T1, T2>::postOrderTraversalRecursive(SubTree<T1, T2>* nextTree, std::vector<Tree<T1, T2>*> &traverse) {
	if (nextTree != nullptr) {
		// Recursively traverse the left (previous) tree
		postOrderTraversalRecursive(nextTree->getPrevSubtree(), traverse);

		// Recursively traverse the right (next) tree
		postOrderTraversalRecursive(nextTree->getNextSubtree(), traverse);
...
template <typename T1, typename T2>
void SubTree<T1, T2>::inOrderSubtreeTraversalRecursive2(SubTree<T1, T2>* nextTree, std::vector<SubTree<T1, T2>*> &traverse) {

	if (nextTree != nullptr) {
		// Recursively traverse the left (previous) tree
		SubTree<T1, T2>* prevSubTree = nextTree->getPrevSubtree();

		if (prevSubTree != nullptr && prevSubTree->getLevel() == nextTree->getLevel() + 1) {
			traverse.push_back(prevSubTree);

			inOrderSubtreeTraversalRecursive2(prevSubTree, traverse);
		}
		...
		if (nextSubTree != nullptr) {
			...
			inOrderSubtreeTraversalRecursive2(nextSubTree, traverse);
		}
	}
}
...
template <typename T1, typename T2>
int	SubTree<T1, T2>::getNodeId() const {
	return id;
}

template <typename T1, typename T2>
int SubTree<T1, T2>::getLevel() const {
	return level;
}

template <typename T1, typename T2>
bool SubTree<T1, T2>::isRoot() const {
	return level == 0;
}

template <typename T1, typename T2>
T1 SubTree<T1, T2>::getKey() const {
	return key;
}

template <typename T1, typename T2>
T2 SubTree<T1, T2>::getValue() const {
	return value;
}

template <typename T1, typename T2>
int SubTree<T1, T2>::getSize() const {
	return size;
}

template <typename T1, typename T2>
int SubTree<T1, T2>::getSubtreeSize() const {
	return subtreeSize;
}

template <typename T1, typename T2>
int SubTree<T1, T2>::getNodeSize() const {
	return nodeSize;
}

template <typename T1, typename T2>
bool SubTree<T1, T2>::isEmpty() const {
	return size == 0;
}

template <typename T1, typename T2>
bool SubTree<T1, T2>::duplicatesAllowed() const {
	return allowDuplicates;
}

template <typename T1, typename T2>
Tree<T1, T2>* SubTree<T1, T2>::getPrevSubtree() const {
	return prevSubtree;
}

template <typename T1, typename T2>
Tree<T1, T2>* SubTree<T1, T2>::getNextSubtree() const {
	return nextSubtree;
}
...
template <typename T1, typename T2>
void SubTree<T1, T2>::setKey(T1 key) {
	this->key = key;
}

template <typename T1, typename T2>
void SubTree<T1, T2>::setValue(T2 value) {
	this->value = value;
}
...
template <typename T1, typename T2>
void SubTree<T1, T2>::setAllowDuplicates(bool allowDuplicates) {
	this->allowDuplicates = allowDuplicates;
}
...
template <typename T1, typename T2>
std::vector<SubTree<T1, T2>*> SubTree<T1, T2>::preOrderTraversal() {
	std::vector<SubTree<T1, T2>*> traverse;
...
template <typename T1, typename T2>
void SubTree<T1, T2>::preOrderTraversal(std::vector<Tree<T1, T2>*> &traverse) {
...
template <typename T1, typename T2>
std::vector<SubTree<T1, T2>*> SubTree<T1, T2>::inOrderTraversal() {
	std::vector<SubTree<T1, T2>*> traverse;
...
template <typename T1, typename T2>
void SubTree<T1, T2>::inOrderTraversal(std::vector<Tree<T1, T2>*> &traverse) {
...
template <typename T1, typename T2>
std::vector<SubTree<T1, T2>*> SubTree<T1, T2>::postOrderTraversal() {
...
template <typename T1, typename T2>
void SubTree<T1, T2>::postOrderTraversal(std::vector<Tree<T1, T2>*> &traverse) {
